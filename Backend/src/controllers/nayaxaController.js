...
const checkIsNayaxaDomain = (req) => {
    const host = req.get('host') || '';
    const origin = req.get('origin') || '';
    const referer = req.get('referer') || '';
    const bodyDomain = req.body?.origin_domain || '';
    return (
        host.includes('nayaxa.my.id') ||
        origin.includes('nayaxa.my.id') ||
        referer.includes('nayaxa.my.id') ||
        bodyDomain.includes('nayaxa.my.id') ||
        host.includes('localhost') ||
        host.includes('127.0.0.1')
    );
};

const sanitizeBapperidaAssistant = (text) => {
...
const nayaxaController = {
    ...
    chat: async (req, res) => {
        ...
        try {
            ...
            const isNayaxaDomain = checkIsNayaxaDomain(req);
            const effectiveNamaInstansi = isNayaxaDomain ? 'Nayaxa' : nama_instansi;
            const cleanPersonaPromptSnippet = (isNayaxaDomain && personaPromptSnippet) 
                ? sanitizeBapperidaAssistant(personaPromptSnippet) 
            ...
            const routingParams = {
                ...
                page_title, baseUrl, fullDate, nama_instansi: effectiveNamaInstansi, 
                personaPromptSnippet: cleanPersonaPromptSnippet, 
            ...
            let responseText = routeResult.text;
            if (isNayaxaDomain) {
                responseText = sanitizeBapperidaAssistant(responseText);
            }
        ...
        try {
            ...
            const onStepCallback = (data) => {
                if (data.type === 'thought') {
                    ...
                    if (isNayaxaDomain) {
                        chunk = sanitizeBapperidaAssistant(chunk);
                    }
            ...
            const [rows] = await dbNayaxa.query(
                ...
                FROM (
                    SELECT 
                        ...
                        s.app_id, 
                        s.user_id, 
                        CONVERT(s.title USING utf8mb4) COLLATE utf8mb4_unicode_ci as title, 
                    ...
                    SELECT 
                        ...
                        h.app_id,
                        h.user_id,
                        CONVERT(SUBSTRING((SELECT content FROM nayaxa_chat_history WHERE session_id = h.session_id ORDER BY id ASC LIMIT 1), 1, 50) USING utf8mb4) COLLATE utf8mb4_unicode_ci as title,
    ...
    getUsageStats: async (req, res) => {
        try {
            ...
            // 3. Map user names from nayaxa_users (dbNayaxa)
            ...
            if (userIds.length > 0) {
                try {
                    ...
                    `, [userIds]);
                    rows.forEach(r => { 
                        userMap[r.id] = r.name || r.username; 
                    });
                } catch (dbErr) {
                    console.warn('[UsageStats] nayaxa_users fetch failed:', dbErr.message);
                }
            }
