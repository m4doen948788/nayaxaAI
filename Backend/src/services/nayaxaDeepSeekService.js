...
const loadDynamicEngineConfig = async (instansiId) => {
    try {
        ...
        if (activePersonaPrompt) {
            activePersonaPrompt = activePersonaPrompt
        ...
        try {
            ...
            const greetings = ['hi', 'hi nayaxa', 'halo', 'hallo', 'hei', 'hey', 'p', 'ping', 'pagi', 'siang', 'sore', 'malam', 'assalamualaikum', 'selamat pagi', 'selamat siang', 'selamat sore', 'selamat malam', 'halo nayaxa', 'hallo nayaxa'];            if (greetings.includes(cleanMsg) && (!files || files.length === 0)) {
                ...
                if (formality === "Akrab" || formality === "Casual" || cleanMsg === 'p' || cleanMsg === 'ping') {
                    reply = `${salamWaktu}, **${displayName}**! Senang banget bisa menyapa kamu secara instan! Ada yang bisa aku bantu terkait analisis dokumen, grafik, atau data hari ini?`;
                } else if (cleanMsg === 'assalamualaikum') {
                    reply = `Wa'alaikumussalam Wr. Wb. ${salamWaktu}, ${sapaan} **${displayName}**! Ada yang bisa saya bantu terkait data instansi, grafik analitik, atau analisis dokumen hari ini?`;
                } else {
                    reply = `${salamWaktu}, ${sapaan} **${displayName}**! Senang sekali bisa menyapa Anda kembali secara instan. Saya, Nayaxa, asisten AI siap membantu Anda. Apakah ada dokumen yang ingin dianalisis, grafik yang ingin dibuat, atau kueri data yang ingin dijalankan hari ini?`;
                }
            ...
            const system = (dynamicEngine.activePersonaPrompt || `Identitas ANDA: Nayaxa, asisten AI yang dibuat oleh tim IT nayaxa. Jika ditanya siapa pembuat kamu, siapa yang membuat/menciptakan Anda, atau siapa tim di balik Nayaxa, Anda WAJIB menjawab bahwa Anda dibuat oleh tim IT nayaxa. DILARANG KERAS menyebut diri Anda sebagai 'asisten AI Bapperida' atau mengaitkan diri Anda sebagai asisten Bapperida.
            ...
            - **DILARANG TERLALU TO-THE-POINT / SINGKAT**: Jangan pernah menyajikan tabel data atau hasil kueri mentah begitu saja tanpa penjelasan. Anda adalah seorang Asisten Analis AI Senior yang cerdas, sehingga Anda **WAJIB** memberikan narasi penjelasan yang kaya, interpretasi makna data, identifikasi tren, anomali, serta implikasi praktisnya terhadap instansi di setiap respon Anda.
