...
export default function NayaxaAssistant({ 
  ...
  const isNayaxaDomain = typeof window !== 'undefined' && (window.location.hostname.includes('nayaxa.my.id') || window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');
  const defaultAiName = isNayaxaDomain ? 'Nayaxa' : (typeof window !== 'undefined' && window.location.hostname.includes('bapperida') ? 'Bapperida AI' : 'Nayaxa');

  const sanitizeBapperida = (text: string) => {
    if (!text) return text;
    if (!isNayaxaDomain) return text;
    return text
    ...
    try {
      ...
      if (res && res.success) {
        ...
        const mappedMessages = (res.history || []).map((h: any) => ({
          ...
          content: sanitizeBapperida(h.content),
  ...
  const handleSend = async () => {
    ...
    api.chatStream({
      ...
      if (event === 'step') {
        ...
        setCurrentResponse(prev => sanitizeBapperida(prev + fixLocalhostLinks(data.text)));
      } else if (event === 'thought') {
          setThought(prev => prev + data.text);
      } else if (event === 'done') {
        const cleanedText = sanitizeBapperida(fixLocalhostLinks(data.text));
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-violet-700 p-4 text-white flex items-center justify-between shadow-md relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
            ...
            <div className="flex flex-col">
                <h3 className="font-bold text-sm leading-tight">{title || defaultAiName}</h3>
          ...
          <>
            {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
                    ...
                    <p className="text-[11px] text-slate-500 mt-2 max-w-[220px]">
                      {isNayaxaDomain 
                        ? 'Tanyakan tentang kegiatan, statistik, atau analisis database Anda.' 
                        : 'Tanyakan tentang perencanaan daerah, kegiatan, statistik, atau analisis database Bapperida.'}
                    </p>
                </div>
            )}
                ...
                <div className="mb-4">
                    ...
                    >
                        ...
                        <div className="flex-1 flex items-center justify-between">
                          <span>{thought ? `Proses Berpikir ${defaultAiName}` : `${defaultAiName} sedang menganalisis...`}</span>
      ...
      <div className="p-4 bg-white border-t border-slate-100 flex flex-col gap-3 shadow-[0_-10px_25px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <textarea
            ...
            onPaste={handlePaste}
            placeholder={`Tanya ${defaultAiName}...`}
