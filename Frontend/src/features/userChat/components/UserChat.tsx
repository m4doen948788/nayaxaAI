...
import { encryptUserMessage, decryptUserMessage } from '../utils/e2eeCrypto';
...
export default function UserChat({ onSwitchToAi, onOpenAuth }: UserChatProps) {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.messages)) {
        const decryptedMessages = await Promise.all(
          res.messages.map(async (msg: Message) => {
            if (msg.message && msg.message.startsWith('E2EE:v1:')) {
              const dec = await decryptUserMessage(msg.message, msg.sender_id, msg.recipient_id);
              return { ...msg, message: dec.text, is_e2ee: dec.isE2EE };
            }
            return { ...msg, is_e2ee: false };
          })
        );
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      // Enkripsi pesan menggunakan protokol E2EE (AES-GCM 256-bit) di sisi klien sebelum dikirim
      const encryptedPayload = await encryptUserMessage(messageText, currentUser.id, selectedContact.id);
          ...
          <>
            ...
            <div className="p-3.5 md:p-4 border-b border-slate-200 bg-white/90 backdrop-blur-md flex items-center justify-between shrink-0 shadow-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:flex items-center gap-1 text-[11px] text-amber-600 font-semibold px-2.5 py-1 bg-amber-50 border border-amber-200 rounded-full">
                  <Clock size={13} /> Auto-delete 3 Jam
                </span>
                <button
              ...
              {loadingMessages ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-3">
                  ...
                  <p className="text-xs text-slate-500 max-w-xs">
                    Kirim pesan pertama Anda kepada <span className="font-semibold text-slate-700">Lawan Bicara</span>. Pesan otomatis terhapus permanen setelah 3 jam.
                  </p>
                </div>
              ) : (
