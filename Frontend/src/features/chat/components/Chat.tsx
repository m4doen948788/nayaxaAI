...
import { Send, Bot, User, Plus, Pin, Paperclip, Mic, Volume2, Sparkles, Search, MoreVertical, ChevronDown, Code2, Square, X, Image as ImageIcon, FileText, Copy, Check, LogIn, UserPlus, LogOut, Share2, Pencil, Trash2, PanelLeft, PanelLeftClose, MessageSquare } from 'lucide-react';
...
const INSTANSI_ID = isNayaxaSite ? 0 : 2;

const sanitizeAssistantIdentity = (text: string) => {
  if (!text) return text;
  return text
    .replace(/asisten\s+ai\s+dari\s+bapperida/gi, 'asisten AI')
    .replace(/asisten\s+ai\s+bapperida/gi, 'asisten AI')
    .replace(/asisten\s+bapperida/gi, 'asisten AI')
    .replace(/analis\s+senior\s+bapperida/gi, 'analis senior');
};
...
export default function Chat() {
  ...
  const loadSession = async (id: string) => {
    ...
    try {
      ...
      if (res.success) setMessages((res.history || []).map((m: any) => ({ ...m, content: sanitizeAssistantIdentity(m.content) })));
    } catch (err) { console.error(err); }
  };
  ...
  const handleSend = async () => {
    ...
    abortControllerRef.current = new AbortController();    try {
      ...
      const res = await api.chatStream(
        ...
        (chunk) => { setStreamingContent(prev => sanitizeAssistantIdentity(prev + chunk)); },
        (thoughtChunk) => { setThinkingThought(prev => prev + thoughtChunk); },
        abortControllerRef.current.signal
      );
      setMessages(prev => [...prev, { role: 'model', content: sanitizeAssistantIdentity(res.text || streamingContent || '*(Tidak ada respons dari Nayaxa)*'), brain_used: res.brain_used }]);
      setStreamingContent('');
      setThinkingThought('');
    ...
    try {
      if (authMode === 'register') {
        ...
        if (!authEmail.trim() || !authPassword) {
          setAuthError('Email/username dan kata sandi wajib diisi.');
          setAuthLoading(false);
          return;
        }

        const usernameCandidate = authEmail.includes('@') ? authEmail.split('@')[0] : authEmail.trim();
        const res = await api.register({
          username: usernameCandidate,
          email: authEmail.includes('@') ? authEmail.trim() : undefined,
          name: authName.trim(),
        ...
        if (res.success && res.user) {
          ...
          setAuthEmail('');
          setAuthPassword('');
          setToastMessage('Akun baru berhasil didaftarkan!');
        } else {
          setAuthError(res.message || 'Gagal mendaftarkan akun baru.');
        }
      } else {
        if (!authEmail.trim() || !authPassword) {
          setAuthError('Silakan masukkan username/email dan kata sandi.');
          setAuthLoading(false);
          return;
        }
        ...
        if (res.success && res.user) {
          ...
          setAuthError(res.message || 'Username/email atau kata sandi tidak sesuai.');
        }
      }
    } catch (err: any) {
      console.error('[Chat Auth Error]', err);
      const msg = err.response?.data?.message || err.message || 'Gagal menghubungi server autentikasi.';
      setAuthError(msg);
    } finally {
      setAuthLoading(false);
    }
            ...
            <div className="bg-white p-2 border border-slate-200 rounded-2xl flex flex-col max-w-4xl mx-auto shadow-2xl transition-all">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <div className="flex-1 px-4 py-2">
                    <textarea ref={inputRef} value={input} rows={1} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} onPaste={handlePaste} placeholder="Tanya Nayaxa..." className="w-full bg-transparent border-none focus:outline-none text-slate-900 text-[16px] resize-none min-h-[24px] max-h-40 py-1 overflow-y-auto custom-scrollbar" />
                  </div>
            ...
            >
              ...
              <form onSubmit={handleAuthSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/20 mt-2 flex items-center justify-center gap-2"
                >
                  {authLoading ? (
                    <>
