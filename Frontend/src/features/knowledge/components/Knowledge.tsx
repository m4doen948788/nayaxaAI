    ...
    <div className="h-full overflow-y-auto p-8 space-y-8 max-w-7xl mx-auto custom-scrollbar">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage the documents and context used by your AI assistant.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <button className="flex items-center gap-2 px-6 py-3 bg-white/5 rounded-2xl border border-white/10 text-slate-400 hover:text-white transition-all text-sm">
          ...
          {filteredKnowledge.map((k, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 bg-indigo-600/20 text-indigo-400 rounded-lg">
                  {k.category || 'General'}
                </span>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button className="p-1.5 text-slate-400 hover:text-white rounded-lg hover:bg-white/5"><Edit3 size={14} /></button>
                  <button className="p-1.5 text-slate-400 hover:text-red-400 rounded-lg hover:bg-red-400/5"><Trash2 size={14} /></button>
                </div>
              </div>
              <p className="text-sm text-slate-200 line-clamp-4 leading-relaxed mb-6">
                {k.content}
              </p>
