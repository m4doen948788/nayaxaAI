...
function AppContent() {
  ...
  return (
    <Router>
      <div className="flex h-screen w-full bg-slate-950 overflow-hidden font-outfit">
        {/* Sidebar */}
        <motion.aside 
          initial={false}
          animate={{ width: isSidebarOpen ? 280 : 80 }}
          className="relative h-full bg-slate-900/50 backdrop-blur-2xl border-r border-white/5 flex flex-col z-50"
        >
          <div className="p-6 flex items-center justify-between">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {isSidebarOpen && (
                ...
                >
                  Nayaxa <span className="text-indigo-400">AI</span>
                </motion.span>
              )}
            </div>
          </div>
          ...
          <div className="p-4 mt-auto border-t border-white/5 space-y-4">
             {/* Account Badge */}
             <div className={`flex items-center gap-3 p-3 rounded-2xl bg-white/5 border border-white/5 transition-all ${!isSidebarOpen ? 'justify-center px-0' : ''}`}>
               ...
               {isSidebarOpen && (
                 <div className="overflow-hidden">
                   <p className="text-sm font-bold text-white truncate">{currentUser?.name || 'Loading...'}</p>
                   <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider truncate">{currentUser?.role || 'User'}</p>
                 </div>
               )}
             </div>
             ...
             <button 
                ...
                className="w-full mt-4 p-3 flex items-center justify-center rounded-2xl bg-white/5 text-slate-400 hover:text-white transition-all"
             >
        ...
        {/* Main Content */}
        <main className="flex-1 relative overflow-hidden bg-gradient-radial from-slate-900 to-slate-950">
